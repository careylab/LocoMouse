...
%
% Future Features:
% * Write a proper input parser.
%
% * Track area features via their centroid, allowing for more robust
% selection and occlusion detection.
%
% * Write the training code in a similar structure so that not many
% arguments need to be passed for the detectors.
%
% * Check if we can load a video file and then post-process it in images or
% check if we can get the videos directly as images.
%
% the data structure is assumed to be as such:
% data.mirror_line: Height where to split the image.
% data.calibration: Either a calibration matrix or the path to load it.
% data.background: Either a background image or the path to load it.
% data.sequence: Either the path to an movie file, or a list of paths to
% image files.
% data.subtract_background: Boolean checking if we want to subtract the
% background to the images/video.
% data.apply_calibration: Boolean checking if we want to calibrate the
% images.
%
...
% Assuming for now a list of images without background and undistorted:
...
% w_point = {model.ear.w};
% rho_point = model.ear.rho;
% detection_threshold_point = model.ear.detection_threshold;
% box_size_point = model.ear.box_size;
Npt = length(w_point);
...
%     imshow(I);
%     hold on
%     plot([BB(1,1,i_images) BB(1,1,i_images)-BB(2,1,i_images)+1]',[mirror_line+BB(1,2,i_images) mirror_line+BB(1,2,i_images)-BB(2,2,i_images)+1]','-*');
%     plot([BB(1,1,i_images) BB(1,1,i_images)-BB(2,1,i_images)+1]',[BB(1,3,i_images) BB(1,3,i_images)-BB(2,3,i_images)+1]','-*');
%     hold off
%     export_fig(gcf,sprintf('bw_test_%05d.png',i_images));

end

C2 = reshape(Centroids,4,[]);C2(1,:) = mean(C2([1 3],:),1);C2(3,:) = [];
% Centroids = C2;clear C2;
% final_tracks = [];
% tracks_tail = [];
% ONG = [];
% tracks_final = [];
...
% [X,Y] = meshgrid(([0.25 0.75]*box_dim(2)),round(occluded_distance/2):occluded_distance:round(0.75*box_dim(1)));% Grid of occlusion points.
% ONG = repmat([Y(:)' 0;X(:)' 0.5*box_dim(2)],1,1,N_images);
...
parfor i_images = 1:N_images
    ...
    %%% FIXME: Images should not be cropped to the exact point but instead
    %%% have a margin of half a bounding box of the feature being filtered.
    %%% Then use the 'valid' option in conv2 to keep the size to the
    %%% desired box.
    if ~isempty(I_cell{1})
    ...
    if l_x_cut > tail_x_threshold && ~all(cellfun(@isempty,w_tail))
        ...
        % Croping the mouse:
        % FIXME: If we stay with this solution of local box coordinates,
        % recode the tailDetectionFilter function to match it!.
    ...
    for i_point = 1:Npt
        ...
        for i_views = 1:2
            ...
            if any(detections(:))
                ...
                if i_views  == 1
                    ...
                    %                     [D2{i_views}, scores{i_views}] = nmsMax([i{i_views} j{i_views}],box_size_point{i_point, i_views}, scores{i_views}','center',0.3);
        ...
        if any(cellfun(@isempty,D2))
            ...
            %             [Pairs, scores_pairs, C{i_point,i_images}] = boxPairings(D2{1}, D2{2}, scores{1}, scores{2}, box_size_point{i_point, i_views}(2), 'bottom', T);
            ...
            %             tracks_joint{i_point, i_images} = [[Pairs scores_pairs]' [NaN(1,size(D2{2},1));D2{2}(:,[2 1])';zeros(size(scores{2}));scores{2}] [D2{1}';NaN(1,size(D2{1},1));scores{1};zeros(size(scores{1}))]];
        ...
        switch i_point
            case 1
                ...
                if ~isempty(weights)
                    %%% FIXME: This could be computed always in the local
                    %%% referential and thus not need any sort of offset
                    %%% operation. The offset would only be needed to
                    %%% transform the coordinates after all per image
                    %%% processing is done.
...
parfor i_images = 1:(N_images-1)
    ...
    for i_point = 1:2
        ...
        %         Pairwise{i_point, i_images} = computePairwiseCostWithImageDifference(tracks_bottom{i_point,i_images}(1:2,:),tracks_bottom_aux{i_point,i_images}(1:2,:),I1,I2,box_size_point{i_point},ONG(:,:,i_images),xvel(i_images)+occluded_distance,alpha_vel);
    end
end
...
for i_tracks = 1:N_tracks
    ...
    parfor i_images = 1:N_images
        ...
        if M(i_tracks,i_images) <= size(tracks_bottom{i_point,i_images},2)
            ...
            if any(candidates)
                ...
                %                 Unary_top{i_tracks,i_images} = [(tracks_joint2{i_images}(end,:).*(tracks_joint2{i_images}(3,:)/mirror_line)) zeros(1,Nong_top)]';
    ...
    parfor i_images = 1:(N_images-1)
        ...
        %         I1 = imread(data_list(i_images,:));
        %         I2 = imread(data_list(i_images,:));
        %
        %         if ~isempty(tracks_joint2{i_images})
        %             x = tracks_joint2{i_images}(1,1)*ones(1,Nong_top);
        %         elseif ~isempty(tracks_joint_2_aux{i_images})
        %             x = tracks_joint_2_aux{i_images}(1,1)*ones(1,Nong_top);
        %         else
        %             x = ones(1,Nong_top);
        %         end
        %
        %         if ~isempty(tracks_joint2{i_images})
        %             W1 = getWindowFromImage(I1,tracks_joint2{i_images}([2 1],1),box_size_point{i_point,1}',true);
        %         else
        %             W1 = uint8(zeros(box_size_point{i_point,1}));
        %         end
        %
        %         if ~isempty(tracks_joint_2_aux{i_images})
        %             W2 = getWindowFromImage(I2,tracks_joint_2_aux{i_images}([2 1],1),box_size_point{i_point,1}',true);
        %         else
        %             W1 = uint8(zeros(box_size_point{i_point,1}));
        %         end
        %
        %         ref_vel = sum(abs(double(W1(:))-double(W2(:))))/sum(double(W1(:)));
        %
        %         Pairwise_top{i_tracks, i_images} = computePairwiseCostWithImageDifference_sideview(tracks_joint2{i_images}([1 3],:),tracks_joint_2_aux{i_images}([1 3],:),I1,I2,box_size_point{i_point,2},ref_vel,[x;nong_vect],occluded_distance_top,alpha_vel);
    end
